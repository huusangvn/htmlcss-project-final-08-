    ...
    add: function (elem, types, handler, data) {
      ...
      while ((type = types[i++])) {
        ...
        if (!handlers) {
          ...
          if (
            !special.setup ||
            special.setup.call(elem, data, namespaces, handler) === false
          ) {
        ...
        if (special.add) {
          var modifiedHandler = special.add.call(
            elem,
            handler,
            data,
            namespaces,
  ...
  jQuery.each(["live", "die"], function (i, name) {
    jQuery.fn[name] = function (types, data, fn) {
      ...
      while ((type = types[i++]) != null) {
        type =
          type === "focus"
            ? "focusin" // focus --> focusin
            : type === "blur"
            ? "focusout" // blur --> focusout
            : type === "hover"
            ? types.push("mouseleave") && "mouseenter" // hover support
            : type;

        if (name === "live") {
          // bind live handler
          jQuery(this.context).bind(
    ...
    clone: function (events) {
      ...
      var ret = this.map(function () {
        if (!jQuery.support.noCloneEvent && !jQuery.isXMLDoc(this)) {
          ...
          return jQuery.clean(
            [html.replace(rinlinejQuery, "").replace(rleadingWhitespace, "")],
            ownerDocument
          )[0];
        } else {
